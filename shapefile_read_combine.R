...
checkDat(shp, inv = inv, species = "species", site = "Site", quad = "Quadrat", year = "z_Year", geometry = "geometry")
...
quadList = list()
i=15
for(i in 99:length(unique(shp_valid$Quadrat))){
  dat_i = shp_valid[shp_valid$Quadrat == unique(shp_valid$Quadrat)[i],]
  inv_i = rep(list(unique(dat_i$z_Year)), length(unique(dat_i$Quadrat)))
  names(inv_i) = unique(dat_i$Quadrat)
  inv_i = as.list(inv_i)
  quadList[[i]] = trackSpp(dat_i, 
                         inv = inv_i,
                         dorm = 1,
                         buff = 0.05,
                         clonal = T,
...
## Save which quads are working
i=1
names = list()
for(i in 1:length(quadList)){
  names[i] = unique(quadList[[i]]$Quadrat)
}

quadList_success <- names(quadList)

quadList_success = quadList_success[quadList_success != "NULL"]

quadList_clean = quadList[quadList != "NULL"]
quadList_clean = lapply(quadList_clean,sf::st_cast, to="MULTIPOLYGON")
pt_out = sf::st_as_sf(data.table::rbindlist(quadList_clean))
